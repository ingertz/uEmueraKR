...
internal abstract class FunctionMethod
{
	...
	protected sealed class ArgTypeList
	{
		...
		public List<ArgType> ArgTypesEnum 
		{ 
			set 
			{
				ArgTypes = new List<_ArgType>();
				foreach(var a in value) ArgTypes.Add(a);
			} 
		}
	...
	private string CheckArgumentTypeEx(string name, IOperandTerm[] arguments)
	{
		...
		for (int idx = 0; idx < argumentTypeArrayEx.Length; idx++)
		{
			...
			if (argsNotMoreThanRule && argsNotLessThanRule)
			{
				if (list.MatchVariadicGroup && vs != null)
				{
					...
					{
						errMsg[idx] = string.Format("{0}: ArgsNotFitExpr", name, arguments.Length, variadicGroupStart, vs.Length);
						continue;
					}
				}
				// 引数の数が有効
				for (int i = 0; i < (variadic ? arguments.Length : Math.Min(arguments.Length, list.ArgTypes.Count)); i++)
				{
					...
					if (arguments[i] == null)
					{
						...
						{
							errMsg[idx] = string.Format("{0}: ArgCanNotBeNull", name, i + 1);
							break;
						}
						else continue;
					}
					...
					if (rule.Ref)
					{
						...
						{
							// キャラ変数ではない
							errMsg[idx] = string.Format("{0}: ArgIsNotCharacterVar", name, i + 1);
							break;
						}
						// 引数の型が違う
						bool error = false;
						...
						switch (dims)
						{
							case 0:
								{
									// 普通の場合
									var err = rule.String ? "ArgIsNotStrVar"
										: rule.Int ? "ArgIsNotIntVar" : "ArgIsNotVar";
									errText = string.Format(err, name, i + 1);
									break;
								}
							case -1:
								{
									// 任意配列の場合
									var err = rule.String ? "ArgIsNotStrArray"
										: rule.Int ? "ArgIsNotIntArray" : "ArgIsNotArray";
									errText = string.Format(err, name, i + 1);
									break;
								}
							default:
								{
									// 1-3次元配列の場合
									var err = rule.String ? "ArgIsNotNDStrArray"
										: rule.Int ? "ArgIsNotNDIntArray" : "ArgIsNotNDArray";
									errText = string.Format(err, name, i + 1, dims);
									break;
								}
						}
						// 引数が引用系
					...
					else if (typeNotMatch)
					{
						...
						// 引数の型が違う
						errMsg[idx] = type == typeof(string) ? string.Format("{0}: ArgIsNotStr", name, i + 1)
							: string.Format("{0}: ArgIsNotInt", name, i + 1);
						break;
					}
				}
			...
			else if (list.OmitStart == -1 && list.ArgTypes.Count > 0 && !variadic)
			{
				...
				if (list.ArgTypes.Count > 0)
					errMsg[idx] = string.Format("{0}: ArgsCountNotMatches", name, list.ArgTypes.Count, arguments.Length);
				else
					errMsg[idx] = string.Format("{0}: ArgsNotNeeded", name);
				continue;
			}
			// 可変長引数
			...
			{
				// 引数が多すぎる
				errMsg[idx] = string.Format("{0}: TooManyFuncArgs", name);
				continue;
			}
			else
			{
				// 引数が足りない
				errMsg[idx] = string.Format("{0}: NotEnoughArgs", name, list.OmitStart < 0 ? list.ArgTypes.Count : list.OmitStart);
				continue;
			}
		}
		...
		{
			sb.Append(string.Format("{0}: NotValidArgsReason", i + 1, errMsg[i]));
			if (i + 1 < errMsg.Length) sb.Append(" | ");
		}
		return string.Format("{0}: NotValidArgs", name, sb.ToString());
	}
		...
		else if (argumentTypeArray != null)
		{
			...
			// return string.Format(Properties.Resources.SyntaxErrMesMethodDefaultArgumentNum0, name);
			{
				if (argumentTypeArray.Length > 0)
					return string.Format("{0}: ArgsCountNotMatches", name, argumentTypeArray.Length, arguments.Length);
				else
					return string.Format("{0}: ArgsNotNeeded", name);
			}
			for (int i = 0; i < argumentTypeArray.Length; i++)
			{
				if (arguments[i] == null)
					...
					return string.Format("{0}: ArgCanNotBeNull", name, i + 1);
				if (argumentTypeArray[i] != arguments[i].GetOperandType())
					...
					return argumentTypeArray[i] == typeof(string) ? string.Format("{0}: ArgIsNotStr", name, i + 1)
							: string.Format("{0}: ArgIsNotInt", name, i + 1);
			}
		}
		return null;
	...
	//実際の計算。
	public virtual long GetIntValue(ExpressionMediator exm, IOperandTerm[] arguments) { throw new ExeEE("{0}: ReturnTypeDifferentOrNotImpelemnt"); }
	public virtual string GetStrValue(ExpressionMediator exm, IOperandTerm[] arguments) { throw new ExeEE("{0}: ReturnTypeDifferentOrNotImpelemnt"); }
	...
	{ throw new ExeEE("{0}: NotImplement"); }
